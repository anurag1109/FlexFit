...
userRoute.patch("/submit_trainer_details", authMiddleWare, async (req, res) => {
...
userRoute.post("/apply", authMiddleWare, async (req, res) => {
  const { name, email, expertise, address, samplePics } = req.body;
  try {
...
userRoute.get("/:id", async (req, res) => {
  try {
    ...
    const { name, email, role, approved, expertise, address, price, _id } =
      user;
    res.send({
      ok: true,
      user: { name, email, role, approved, expertise, address, price, _id },
    });
  } catch (error) {
