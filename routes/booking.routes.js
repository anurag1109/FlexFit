...
BookingRouter.post('/book', authMiddleWare,async (req, res) => {
  ...
  try {
    ...
    if (!trainer){
      return res.status(400).json({ message: 'Invalid trainer or client ID', ok:false });
    }
    // Create the booking
...
BookingRouter.get('/requests/:status', authMiddleWare, async (req, res) => {
  try {
    ...
    const bookings = await BookingModel.find({ trainer: trainerId, status: req.params.status }).populate('client', 'name email');
...
BookingRouter.get('/requests', authMiddleWare, async (req, res) => {
  try {
    ...
    // Find all booking requests for the logged-in trainer from the database
    const bookings = await BookingModel.find({ client: clientId }).populate('trainer', 'name email');
    res.json({ ok: true, bookings });
  } catch (err) {
    res.status(500).send({ error: err.message, mssg: 'Server Error', ok: false });
  }
...
BookingRouter.post('/:bookingId/notifications', authMiddleWare, async (req, res) => {
  try {
    ...
    const booking = await BookingModel.findOne({ _id: bookingId, trainer: from });
    if (!booking) {
      return res.status(404).json({ error: 'Booking not found' });
    }
